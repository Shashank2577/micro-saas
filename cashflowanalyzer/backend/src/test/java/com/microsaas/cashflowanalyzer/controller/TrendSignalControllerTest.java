...
@WebMvcTest(TrendSignalController.class)
class TrendSignalControllerTest {
    ...
    void list_ShouldReturnOk() throws Exception {
        when(service.list()).thenReturn(List.of(new TrendSignal()));
        mockMvc.perform(get("/api/v1/cashflow/trend-signals"))
