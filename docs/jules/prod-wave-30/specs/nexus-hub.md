# NexusHub — Production Progression Spec (Wave Upgrade)

## 1) Product Intent
- **App:** `nexus-hub`
- **Domain:** Platform Orchestration
- **Outcome:** Central app registry, event bus governance, workflow orchestration, and integration contracts
- **Primary actors:** Platform admin, Ops engineer, Solution architect

## 2) Current Maturity Snapshot (from repository audit)
- **Score:** 45
- **Tier:** Alpha
- **Depth:** Shallow
- **Known structural risks:** no-integration-manifest, no-backend-tests, no-frontend-tests, no-backend-dockerfile, no-readme
- **Low-hanging fruits before/while implementation:**
  ...
  - Create normalized integration-manifest contract and register with Nexus Hub.

## 3) Scope for This Jules Session (must be fully implemented)
- [ ] 1. apps: backend service + controller + frontend page + tests
- [ ] 2. contracts: backend service + controller + frontend page + tests
- [ ] 3. subscriptions: backend service + controller + frontend page + tests
- [ ] 4. workflows: backend service + controller + frontend page + tests
- [ ] 5. runs: backend service + controller + frontend page + tests
- [ ] 6. health: backend service + controller + frontend page + tests
...
| name | varchar(180) | RegisteredApp display name |
...
| id | UUID | Primary key |
| tenant_id | UUID | Tenant isolation key |
| name | varchar(180) | EventContract display name |
| status | varchar(40) | Lifecycle status enum |
| metadata_json | jsonb | Extensible structured attributes |
| created_at | timestamptz | Creation timestamp |
| updated_at | timestamptz | Last updated timestamp |
...
| id | UUID | Primary key |
| tenant_id | UUID | Tenant isolation key |
| name | varchar(180) | WorkflowDefinition display name |
| status | varchar(40) | Lifecycle status enum |
| metadata_json | jsonb | Extensible structured attributes |
| created_at | timestamptz | Creation timestamp |
| updated_at | timestamptz | Last updated timestamp |
...
| id | UUID | Primary key |
| tenant_id | UUID | Tenant isolation key |
| name | varchar(180) | WorkflowRun display name |
| status | varchar(40) | Lifecycle status enum |
| metadata_json | jsonb | Extensible structured attributes |
| created_at | timestamptz | Creation timestamp |
| updated_at | timestamptz | Last updated timestamp |
...
| id | UUID | Primary key |
| tenant_id | UUID | Tenant isolation key |
| name | varchar(180) | Subscription display name |
| status | varchar(40) | Lifecycle status enum |
| metadata_json | jsonb | Extensible structured attributes |
| created_at | timestamptz | Creation timestamp |
| updated_at | timestamptz | Last updated timestamp |
...
| id | UUID | Primary key |
| tenant_id | UUID | Tenant isolation key |
| name | varchar(180) | IntegrationHealth display name |
| status | varchar(40) | Lifecycle status enum |
| metadata_json | jsonb | Extensible structured attributes |
| created_at | timestamptz | Creation timestamp |
| updated_at | timestamptz | Last updated timestamp |
...
- GET /api/v1/nexus/registered-apps
- POST /api/v1/nexus/registered-apps
- GET /api/v1/nexus/registered-apps/{id}
- PATCH /api/v1/nexus/registered-apps/{id}
- POST /api/v1/nexus/registered-apps/{id}/validate
- GET /api/v1/nexus/event-contracts
- POST /api/v1/nexus/event-contracts
- GET /api/v1/nexus/event-contracts/{id}
- PATCH /api/v1/nexus/event-contracts/{id}
- POST /api/v1/nexus/event-contracts/{id}/validate
- GET /api/v1/nexus/workflow-definitions
- POST /api/v1/nexus/workflow-definitions
- GET /api/v1/nexus/workflow-definitions/{id}
- PATCH /api/v1/nexus/workflow-definitions/{id}
- POST /api/v1/nexus/workflow-definitions/{id}/validate
- GET /api/v1/nexus/workflow-runs
- POST /api/v1/nexus/workflow-runs
- GET /api/v1/nexus/workflow-runs/{id}
- PATCH /api/v1/nexus/workflow-runs/{id}
- POST /api/v1/nexus/workflow-runs/{id}/validate
- GET /api/v1/nexus/subscriptions
- POST /api/v1/nexus/subscriptions
- GET /api/v1/nexus/subscriptions/{id}
- PATCH /api/v1/nexus/subscriptions/{id}
- POST /api/v1/nexus/subscriptions/{id}/validate
- POST /api/v1/nexus/ai/analyze
- POST /api/v1/nexus/ai/recommendations
- POST /api/v1/nexus/workflows/execute
- GET /api/v1/nexus/health/contracts
- GET /api/v1/nexus/metrics/summary

## 6) Service Layer Contract
Implement the following service modules with explicit method-level unit tests:
- appsService: create/update/list/getById/delete/validate/simulate
- contractsService: create/update/list/getById/delete/validate/simulate
- subscriptionsService: create/update/list/getById/delete/validate/simulate
- workflowsService: create/update/list/getById/delete/validate/simulate
- runsService: create/update/list/getById/delete/validate/simulate
- healthService: create/update/list/getById/delete/validate/simulate
...
- Add integration-manifest.json with emits/consumes/capabilities/health endpoint.
...
- Backend:
  - `mvn -pl nexus-hub/backend clean verify`
- Frontend:
  - `npm --prefix nexus-hub/frontend test`
  - `npm --prefix nexus-hub/frontend run build`
- Contract checks:
