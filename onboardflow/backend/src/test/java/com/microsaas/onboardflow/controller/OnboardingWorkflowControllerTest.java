...
@WebMvcTest(controllers = OnboardingWorkflowController.class)
public class OnboardingWorkflowControllerTest {
    ...
    public void testGetAll() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/onboardflow/workflows")
    ...
    public void testCreate() throws Exception {
        ...
        OnboardingWorkflow response = new OnboardingWorkflow();
        response.setName("Test");

        when(service.create(eq(tenantId), any(OnboardingWorkflowRequest.class))).thenReturn(response);

        mockMvc.perform(post("/api/v1/onboardflow/workflows")
                .header("X-Tenant-ID", tenantId.toString()).with(csrf())
