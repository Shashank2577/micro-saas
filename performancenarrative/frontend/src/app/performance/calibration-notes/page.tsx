...
export default function CalibrationNotesPage() {
  const [items, setItems] = useState<CalibrationNotes[]>([]);
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get("/performance/calibration-notes");
      setItems(data);
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Calibration Notes</h1>
