...
export default function GoalEvidencesPage() {
  const [items, setItems] = useState<GoalEvidences[]>([]);
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get("/performance/goal-evidences");
      setItems(data);
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Goal Evidences</h1>
