...
class RiskScoreControllerTest {
    ...
    void testCreate() throws Exception {
        ...
        when(service.create(any(RiskScore.class))).thenReturn(entity);

        mockMvc.perform(post("/api/v1/contracts/risk-scores")
    ...
    void testGetById() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/contracts/risk-scores/{id}", id)
                .header("X-Tenant-ID", UUID.randomUUID().toString()))
                .andExpect(status().isOk());
    }
}
