...
export function CompatibilityReportList() {
  const { data, error, isLoading } = useSWR('/api/v1/api-evolution/compatibility-reports', fetcher);
  const [formData, setFormData] = useState({ name: '', status: 'GENERATED', metadataJson: '{}' });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/v1/api-evolution/compatibility-reports', {
      method: 'POST',
      headers: {
