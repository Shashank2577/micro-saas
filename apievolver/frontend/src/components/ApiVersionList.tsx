...
export function ApiVersionList() {
  const { data, error, isLoading } = useSWR('/api/v1/api-evolution/api-versions', fetcher);
  const [formData, setFormData] = useState({ name: '', status: 'PUBLISHED', metadataJson: '{}' });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch('/api/v1/api-evolution/api-versions', {
      method: 'POST',
      headers: {
  ...
  if (error) return <div className="text-red-500">Failed to load API Versions.</div>;
  if (isLoading) return <div className="text-gray-500">Loading API Versions...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded shadow overflow-hidden">
        ...
        {(!data || data.length === 0) && (
          <div className="p-4 text-gray-500 text-center">No API Versions found.</div>
        )}
      </div>
    </div>
  );
